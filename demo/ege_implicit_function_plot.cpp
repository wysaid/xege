	...
	for (int t = 1; is_run(); t++) {
		...
		for (int i = 0; i < 860; i++)
			for (int j = 0; j < 640; j++) {
				...
				if (value > 0) flag[i][j] = 1;	// 正数
				if (value == 0) flag[i][j] = 2;	// 零点
				if (value < 0) flag[i][j] = 3;	// 负数
				if (!e.isValid()||isnan(e.value())||isinf(e.value())) flag[i][j] = 4;	// 计算结果不合法
			}
		// 绘制图像
		...
		for (int i = 1; i < 860 - 1; i++)
			for (int j = 1; j < 640 - 1; j++) {
				...
				s2 += (flag[i - 1][j - 1] == 3) +
				      ...
				      (flag[i + 1][j + 1] == 3);
				// 如果有正有负就标记为零点
				if (s1 && s2) putpixel(i, j, EGERGB(r, g, b));
			}
	}
	close_console();
	closegraph();
